...
namespace Destructible2D
{
	...
	public partial class D2dDestructible : MonoBehaviour
	{
		...
		public bool TrySplit(D2dRect splitRect)
		{
			...
			if (splitRect.MinX < 0) splitRect.MinX = 0;
			if (splitRect.MinY < 0) splitRect.MinY = 0;
			if (splitRect.MaxX > alphaWidth ) splitRect.MaxX = alphaWidth;
			if (splitRect.MaxY > alphaHeight) splitRect.MaxY = alphaHeight;
			...
			D2dFloodfill.FastFind(alphaData, alphaWidth, alphaHeight, lapRect);
			...
			switch (D2dFloodfill.IslandLayout)
			{
				case D2dFloodfill.Layout.Whole:
				{
					...
					{
						D2dFloodfill.FeatherIslands(SplitFeather, alphaHeight);
						...
						for (var i = D2dFloodfill.IslandsA.Count - 1; i >= 0; i--)
						{
							...
							island.Submit(splitGroup, SplitFeather, alphaWidth, alphaHeight);
						}
