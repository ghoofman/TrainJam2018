...
namespace Destructible2D
{
	public static partial class D2dFloodfill
	{
		...
		private static List<FeatheredRow> featheredRows = new List<FeatheredRow>();
		...
		public static void FeatherIslands(int size, int alphaHeight)
		{
			for (var i = 0; i < alphaHeight; i++)
			{
				if (featheredRows.Count == i)
				{
					featheredRows.Add(new FeatheredRow());
				}
				else
				{
					featheredRows[i].Lines.Clear();
				}
			}

			FeatherLines(IslandsA, size, alphaHeight);
			FeatherLines(IslandsB, size, alphaHeight);
			FeatherLines(IslandsC, size, alphaHeight);
		}

		private static void FeatherLines(List<Island> islands, int size, int alphaHeight)
		{
			for (var i = islands.Count - 1; i >= 0; i--)
			{
				var island = islands[i];

				for (var j = island.Lines.Count - 1; j >= 0; j--)
				{
					var line = island.Lines[j];
					var y    = line.Y;

					FeatherLine(line, y, alphaHeight);

					for (var k = 1; k < size; k++)
					{
						FeatherLine(line, y - k, alphaHeight);
						FeatherLine(line, y + k, alphaHeight);
					}
				}
			}
		...
		private static void FeatherLine(Line line, int y, int alphaHeight)
		{
			if (y >= 0 && y < alphaHeight)
			{
				featheredRows[y].Lines.Add(line);
			}
		}
